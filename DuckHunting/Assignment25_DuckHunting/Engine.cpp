...
{
	// Start of the frame
	m_window->clear({ 61,192,255,255 });        // clear previous content
...
void CEngine::KeyPressedListner(sf::Event event)
{
	...
	{
		if (m_spawnManager->GetGameState() != EGameState::MainMenu && m_spawnManager->GetGameState() != EGameState::GameOver)
		{
			m_spawnManager->SetGameState(EGameState::MainMenu); // if game is running , go to menu
			m_spawnManager->ResetGameProperties();
		}
		else
		{
